...
\`lettabot pairing approve whatsapp ${code}\``;
  ...
  async sendFile(file: OutboundFile): Promise<{ messageId: string }> {
    ...
    const payload = file.kind === 'image'
      ? { image: { url: file.filePath }, caption: file.caption }
      : { document: { url: file.filePath }, fileName: basename(file.filePath), caption: file.caption };
  ...
  async addReaction(chatId: string, messageId: string, emoji: string): Promise<void> {
    ...
    if (targetJid.includes('@lid')) {
      if (targetJid === this.selfChatLid && this.myNumber) {
        ...
        console.error(`[WhatsApp] Cannot react to unknown LID: ${targetJid}`);
        throw new Error('Cannot send to unknown LID - no mapping found');
      }
    }

    const key = { remoteJid: targetJid, fromMe: false, id: messageId };
    await this.sock.sendMessage(targetJid, { react: { text: emoji, key } });
  }
