...
export class MessageHookRunner {
  private moduleCache = new Map<string, HookModule | null>();
  ...
  private async loadModule(file: string): Promise<HookModule | null> {
    ...
    if (!existsSync(resolved)) {
      ...
      this.moduleCache.set(resolved, null);
      return null;
    }
    ...
    try {
      ...
      log.warn(`Failed to load ${resolved}:`, err instanceof Error ? err.message : err);
      this.moduleCache.set(resolved, null);
      return null;
    }
  }
